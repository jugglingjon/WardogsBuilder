...
describe('moving a selection', () => {
  ...
  it('carries the build region with the FOB', () => {
    ...
    const edge = model.addPiece({ type: 'hesco_block_small', x: 100, y: 50, z: 0 });
  ...
  it('refuses to push a piece out of the region', () => {
    const { model } = modelWithFob();
    const block = model.addPiece({ type: 'hesco_block_small', x: 102, y: 50, z: 0 });
    expect(canMove(model, [block.id], { dx: 1 }).reasons).toContain(REASON.OUT_OF_REGION);
  });

  it('refuses to leave a piece floating', () => {
    const { model } = modelWithFob();
    pad(model, 'hesco_block_small', 10, 10, 2, 1);
    const top = model.addPiece({ type: 'hesco_block_small', x: 10, y: 10, z: 1 });
...
describe('pasting', () => {
  it('accepts a group that fits', () => {
    ...
    expect(canPlaceAll(model, [
      { type: 'hesco_block_small', x: 20, y: 20, z: 0 },
      { type: 'hesco_block_small', x: 21, y: 20, z: 0 }
    ]).ok).toBe(true);
  });
  ...
  it('lets pasted pieces support each other', () => {
    ...
    expect(canPlaceAll(model, [
      { type: 'hesco_block_small', x: 20, y: 20, z: 0 },
      { type: 'hesco_block_small', x: 20, y: 20, z: 1 }
    ]).ok).toBe(true);
  });

  it('is all or nothing when one piece of the group collides', () => {
    const { model } = modelWithFob();
    model.addPiece({ type: 'hesco_block_small', x: 21, y: 20, z: 0 });
    expect(canPlaceAll(model, [
      { type: 'hesco_block_small', x: 20, y: 20, z: 0 },
      { type: 'hesco_block_small', x: 21, y: 20, z: 0 }
    ]).ok).toBe(false);
...
describe('selection geometry', () => {
  it('measures the box around a selection', () => {
    ...
    expect(boundsOfPieces(model, [a.id, b.id]))
      .toMatchObject({ x0: 10, y0: 10, z0: 0, x1: 16, y1: 16, z1: 2 });
  });
});
